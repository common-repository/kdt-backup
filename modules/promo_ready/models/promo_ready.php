<?php
class promo_readyModelBcp extends modelBcp {
	private $_apiUrl = '';
	...
	public function saveUsageStat($code) {
		$query = '';
		$update = dbBcp::exist('@__usage_stat', 'code', $code);
...
//                outeBcp($allStat);
	...
	public function getUserStatsCount() {
		...
		return (int) dbBcp::get($query, 'one');
	}
        public function checkAndSend(){
             
            $res = frameBcp::_()->getTable("usage_stat")->get("id",'`visits`>9');
            if(!empty($res)){
                $this->sendUsageStat();
            }
        }
