...
abstract class controllerBcp {
	...
	protected function _createModel($name = '') {
		if(empty($name)) $name = $this->getCode();
		$parentModule = frameBcp::_()->getModule( $this->getCode() );
		$className = '';
		if(import($parentModule->getModDir(). 'models'. DS. $name. '.php')) {
			$className = toeGetClassNameBcp($name. 'Model');
		}
	...
	protected function _createView($name = '') {
		...
		if(import($parentModule->getModDir(). 'views'. DS. $name. '.php')) {
			$className = toeGetClassNameBcp($name. 'View');
		}
	 ...
	 * @example :
	 return array(
			S_METHODS => array(
				'save' => array(BCP_ADMIN),
				'remove' => array(BCP_ADMIN),
				'restore' => BCP_ADMIN,
			),
			S_USERLEVELS => array(
	 ...
	 * Can be used on of sub-array - BCP_METHODS or BCP_USERLEVELS
	 */
