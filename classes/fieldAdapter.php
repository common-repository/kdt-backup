...
class fieldAdapterBcp {
    ...
    static public function _($fieldOrValue, $method, $type) {
        if(method_exists('fieldAdapterBcp', $method)) {
            switch($type) {
                case self::DB:
    ...
    static public function userFieldDestHtml($field) {
        $field->htmlParams['optionsBcp'] = array();
    ...
    static public function userFieldDestToDB($value) {
        return utilsBcp::jsonEncode($value);
    }
    static public function userFieldDestFromDB($value) {
        return utilsBcp::jsonDecode($value);
    }
    static public function taxDataHtml($field) {
        ...
        $categories = frameBcp::_()->getModule('products')->getCategories();
        $brands = frameBcp::_()->getModule('products')->getBrands();
        $cOptions = array();
        $bOptions = array();
        if(!empty($categories)) {
        ...
        return '<div>'. langBcp::_('Apply To'). '
            <div id="tax_address">
                <b>'. langBcp::_('Address'). '</b><br />
                '. langBcp::_('Destination'). ':'. htmlBcp::checkboxlist('params[dest]', array('optionsBcp' => $listOfDest)). '<br />
                '. langBcp::_('Country'). ':'. htmlBcp::countryList('params[country]', array('notSelected' => true, 'value' => $field->value['country'])). '<br />
            </div>
            <div id="tax_category">
                <b>'. langBcp::_('Categories'). '</b><br />
                '. (empty($cOptions) ? langBcp::_('You have no categories') : htmlBcp::checkboxlist('params[categories][]', array('optionsBcp' => $cOptions))). '<br />
                    <b>'. langBcp::_('Brands'). '</b><br />
                '. (empty($bOptions) ? langBcp::_('You have no brands') : htmlBcp::checkboxlist('params[brands][]', array('optionsBcp' => $bOptions))). '<br />
            </div>
            <div>'. langBcp::_('Tax Rate').': '. htmlBcp::text('params[rate]', array('value' => $field->value['rate'])).'</div>
            <div>'. langBcp::_('Absolute').': '. htmlBcp::checkbox('params[absolute]', array('checked' => $field->value['absolute'])).'</div>
        </div>';
    }
    ...
    static public function getCountries($notSelected = false) {
        ...
        if(empty($options[ $notSelected ])) {
            ...
            if(empty(self::$countries))
                self::$countries = self::getCachedCountries();
            if($notSelected) {
				$options[ $notSelected ][0] = is_bool($notSelected) ? langBcp::_('Not selected') : langBcp::_($notSelected);
			}
    ...
    static public function getStates($notSelected = false) {
        ...
        if(empty($options[ $notSelected ])) {
            ...
            if($notSelected) {
				...
				$options[ $notSelected ][0] = array('name' => langBcp::_( $notSelectedLabel ), 'country_id' => NULL);
			}
    ...
    static public function getExtraFieldOptions($field_id) {
        ...
        $options = frameBcp::_()->getModule('optionsBcp')->getHelper()->getOptions($field_id);
        if (!empty($options)) {
    ...
    static public function getFieldAttributes($params){
        ...
        if (!empty($params->attr)) {
            foreach ($params->attr as $key=>$value) {
                $output .= langBcp::_($key).':<br />';
                $output .= htmlBcp::text('params[attr]['.$key.']',array('value'=>$value)).'<br />';
            }
        } else {
                $output .= langBcp::_('class').':<br />';
                $output .= htmlBcp::text('params[attr][class]',array('value'=>'')).'<br />';
                $output .= langBcp::_('id').':<br />';
                $output .= htmlBcp::text('params[attr][id]',array('value'=>'')).'<br />';
        }
        return $output;
    }
    /**
    ...
    static function productFieldCategories($field){
        if(!empty($field->htmlParams['optionsBcp']))
            return;
        ...
        if(!empty($categories)) {
            if(!is_array($field->value['categories']))
                    $field->value['categories'] = array();
            $field->htmlParams['optionsBcp'][0] = in_array(0,$field->value['categories'])?langBcp::_('Deselect All'):langBcp::_('Select All');
            foreach($categories as $c) {
                $field->htmlParams['optionsBcp'][$c->term_taxonomy_id] = $c->cat_name;
            }
        }*/
    }
	...
	static public function getCachedCountries($clearCache = false) {
		if(empty(self::$countries) || $clearCache)
			self::$countries = frameBcp::_()->getTable('countries')->getAll('id, name, iso_code_2, iso_code_3');
	...
	static public function getCachedStates($clearCache = false) {
		if(empty(self::$states) || $clearCache)
			self::$states = frameBcp::_()->getTable('states')
				->leftJoin( frameBcp::_()->getTable('countries'), 'country_id' )
