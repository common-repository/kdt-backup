...
class dbBcp {
     ...
     * #__  Wordpress prefix
     * ^__  Store plugin tables prefix (@see BCP_DB_PREF if config.php)
    ...
    static public function prepareQuery($query) {
        ...
        return str_replace(
                array('#__', '^__', '@__'), 
                array($wpdb->prefix, BCP_DB_PREF, $wpdb->prefix. BCP_DB_PREF),
                $query);
    }
    ...
    static public function timeToDate($timestamp = 0) {
        if($timestamp) {
            if(!is_numeric($timestamp))
                $timestamp = dateToTimestampBcp($timestamp);
    ...
    static public function dateToTime($date) {
        ...
        if(strpos($date, BCP_DATE_DL)) return dateToTimestampBcp($date);
        $arr = explode('-', $date);
        return dateToTimestampBcp($arr[2]. BCP_DATE_DL. $arr[1]. BCP_DATE_DL. $arr[0]);
    }
