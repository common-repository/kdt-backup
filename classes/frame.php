...
class frameBcp {
    ...
    public function __construct() {
        $this->_res = toeCreateObjBcp('response', array());
        load_theme_textdomain('bcp',  BCP_DIR . 'lang');
        
    }
    static public function getInstance() {
        static $instance;
        if(!$instance) {
            $instance = new frameBcp();
        }
        return $instance;
    }
        ...
        if($activeModules) {
            foreach($activeModules as $m) {
                ...
                if(!empty($m['ex_plug_dir'])) {
                    $moduleLocationDir = utilsBcp::getExtModDir( $m['ex_plug_dir'] );
                }

                if(is_dir($moduleLocationDir. $code)) {
                    $this->_allModules[$m['code']] = 1;
                    if((bool)$m['active']) {
                        importClassBcp($code. strFirstUp(BCP_CODE), $moduleLocationDir. $code. DS. 'mod.php');
                        $moduleClass = toeGetClassNameBcp($code);
    ...
    public function init() {
		...
		modInstallerBcp::checkActivationMessages();
        ...
        register_activation_hook(  BCP_DIR. DS. BCP_MAIN_FILE, array('utilsBcp', 'activatePlugin')  ); //See classes/install.php file
        register_deactivation_hook(BCP_DIR. DS. BCP_MAIN_FILE, array('utilsBcp', 'deactivatePlugin'));
        
        add_action('admin_notices', array('errorsBcp', 'displayOnAdmin'));
    ...
    public function checkPermissions($code, $action) {
        if($this->havePermissions($code, $action))
            ...
            exit(langBcp::_e('You have no permissions to view this page'));
        }
    }
    /**
    ...
    public function havePermissions($code, $action) {
        ...
        if($mod) {
            ...
            if(!empty($permissions)) {  // Special permissions
                if(isset($permissions[BCP_METHODS]) 
                    && !empty($permissions[BCP_METHODS])
                    
                ) {
                    foreach($permissions[BCP_METHODS] as $method => $permissions) {   // Make case-insensitive
                        $permissions[BCP_METHODS][strtolower($method)] = $permissions;
                    }
                    if(array_key_exists($action, $permissions[BCP_METHODS])) {        // Permission for this method exists
                        $currentUserPosition = frameBcp::_()->getModule('user')->getCurrentUserPosition();
                        
                        if((is_array($permissions[ BCP_METHODS ][ $action ]) && !in_array($currentUserPosition, $permissions[ BCP_METHODS ][ $action ]))
                            || (!is_array($permissions[ BCP_METHODS ][ $action ]) && $permissions[BCP_METHODS][$action] != $currentUserPosition)
                        ) {
                            $res = false;
                        }
                    }
                }
                if(isset($permissions[BCP_USERLEVELS])
                    && !empty($permissions[BCP_USERLEVELS])
                ) {
                    $currentUserPosition = frameBcp::_()->getModule('user')->getCurrentUserPosition();
                  
                   
                    foreach($permissions[BCP_USERLEVELS] as $userlevel => $methods) {
                        if(is_array($methods)) {
    ...
    protected function _extractTables($tablesDir = BCP_TABLES_DIR) {
    ...
    protected function _extractTable($tableName, $tablesDir = BCP_TABLES_DIR) {
        importClassBcp('noClassNameHere', $tablesDir. $tableName. '.php');
        $this->_tables[$tableName] = tableBcp::_($tableName);
     ...
     * @return object table
     * @example frameBcp::_()->getTable('products')->getAll()
     */
...
//                outBcp($s);
    ...
    public function isTplEditor() {
        $tplEditor = reqBcp::getVar('tplEditor');
