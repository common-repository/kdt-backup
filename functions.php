 ...
 */
if(!function_exists('dateToTimestampBcp')) {
    function dateToTimestampBcp($date) {
        if(empty($a))
            return false;
        $a = explode(BCP_DATE_DL, $date);
...
if(!function_exists('importClassBcp')) {
    function importClassBcp($class, $path = '') {
        if(!class_exists($class)) {
            if(!$path) {
				$classFile = $class;
				if(strpos(strtolower($classFile), BCP_CODE) !== false) {
					$classFile = preg_replace('/'. BCP_CODE. '/i', '', $classFile);
				}
                $path = BCP_CLASSES_DIR. $classFile. '.php';
			}
            return import($path);
            ...
            /*if(class_exists('ReflectionClass')) {   //ReflectionClass supported begining from php5
                ...
                if(strpos($classFile, BCP_DIR) === false) {   //Class is not in our plugin directory
                    ...
                    if(!empty($plugins)) {
                        for($i = 0; $i < count($plugins); $i++) {
                            if(strpos($plugins[$i], BCP_PLUG_NAME) !== false) {   //Let's remove our plugin from list of active plugins
                                unset($plugins[$i]);
                            }
                        }
 ...
 */
if(!function_exists('toeGetClassNameBcp')) {
    function toeGetClassNameBcp($class) {
        $className = '';
		if(class_exists($class. strFirstUp(BCP_CODE))) 
			$className = $class. strFirstUp(BCP_CODE);
		else if(class_exists(BCP_CLASS_PREFIX. $class)) 
            $className = BCP_CLASS_PREFIX. $class;
		else
            $className = $class;
        return $className;
    } 
 ...
 */
if(!function_exists('toeCreateObjBcp')) {
    function toeCreateObjBcp($class, $params) {
        $className = toeGetClassNameBcp($class);
        $obj = NULL;
 ...
 * @return array
 */
if(!function_exists('prepareParamsBcp')) {
	function prepareParamsBcp(&$d=array(), &$options = array()) {
		if (!empty($d['params'])) {
			...
			if (is_array($d['params'])) {
				$params = utilsBcp::jsonEncode($d['params']);
		...
		if(isset($d['code'])) {
			if ($d['code'] == '') {
				$d['code'] = prepareFieldCodeBcp($d['label']).'_'.rand(0, 9999999);
			}
		}
		return $d;
	}
}
if(!function_exists('prepareFieldCodeBcp')) {
	function prepareFieldCodeBcp($string) {   
