...
class errorsBcp {
    ...
    static public function setSession($error) {
        ...
        if(is_array($error))
            ...
            $sesErrors[] = $error;
        reqBcp::setVar('sesErrors', $sesErrors, 'session');
    }
    static public function init() {
        $bcpErrors = reqBcp::getVar('bcpErrors');
        if(!empty($bcpErrors)) {
            if(!is_array($bcpErrors)) {
                $bcpErrors = array( $bcpErrors );
            }
            $bcpErrors = array_map('htmlspecialchars', array_map('stripslashes', array_map('trim', $bcpErrors)));
            if(!empty($bcpErrors)) {
                self::$current = $bcpErrors;
                add_filter('the_content', array('errorsBcp', 'appendErrorsContent'), 99999);
            }
        }
    }
    ...
    static public function getSession() {
        return reqBcp::getVar('sesErrors', 'session');
    }
    static public function clearSession() {
        reqBcp::clearVar('sesErrors', 'session');
    }
