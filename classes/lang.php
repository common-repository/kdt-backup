...
class langBcp {
	...
	static public function extract($d = array('dir' => '', 'langBcp' => '')) {
		$data = array();
		if(isset($d['dir']) && !empty($d['dir']))
			$langDirPath = $d['dir'];
		else if(isset($d['langBcp']) && !empty($d['langBcp']))
			$langDirPath = BCP_LANG_DIR. $d['langBcp']. DS;
		else
			$langDirPath = BCP_LANG_DIR. BCP_WPLANG. DS;
