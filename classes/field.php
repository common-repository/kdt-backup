...
class fieldBcp {
	 ...
	 */
	public $adapt = array('htmlBcp' => '', 'dbFrom' => '', 'dbTo' => '');
	/**
	...
	public function setValue($value, $fromDB = false) {
		if(isset($this->adapt['dbFrom']) && $this->adapt['dbFrom'] && $fromDB)
			$value = fieldAdapterBcp::_($value, $this->adapt['dbFrom'], fieldAdapterBcp::DB);
		$this->value = $value;
	}
	...
	public function drawHtml($tag, $id) {
		if(method_exists('htmlBcp', $this->html)) {
			$method = $this->html;
			...
			if($this->adapt['html']) {
				fieldAdapterBcp::_($this, $this->adapt['html'], fieldAdapterBcp::HTML);
			}
			$params = $this->processParams($tag, $id);
			if ($params != '')
				return $params;
			if ($this->name == 'default_value') {
				$optionsFromDb = frameBcp::_()->getModule('optionsBcp')->getHelper()->getOptions($id);
				if (!empty($optionsFromDb)) {
					$options = array(0 => langBcp::_('Select'));
					foreach($optionsFromDb as $k => $v)
						$options[$k] = $v;
					$method = 'selectbox';
					$this->htmlParams['optionsBcp'] = $options;
				}
			}
			$htmlContent = htmlBcp::$method($this->name, $this->htmlParams);
			if(!empty($this->errorEl)) {
				if(is_bool($this->errorEl))
					$errorEl = '<div class="toeErrorForField toe_'. htmlBcp::nameToClassId($this->name). '"></div>';
	...
	public function displayValue() {
		...
		switch($this->html) {
			case 'countryList':
				$value = fieldAdapterBcp::displayCountry($this->value);
				break;
			case 'statesList':
				$value = fieldAdapterBcp::displayState($this->value);
				break;
			case 'checkboxlist':
				$options = $this->getHtmlParam('optionsBcp');
				$value = array();
	...
	public function viewField($name, $fieldValue = '') {
		$method = $this->html;
		$options = frameBcp::_()->getModule('optionsBcp')->getHelper()->getOptions($this->id);
		$attrs = '';
		...
		$params = array('optionsBcp'=>$options, 'attrs' => $attrs, 'value' => $fieldValue, 'checked' => $checked);
		$output = '';
		if(method_exists('htmlBcp', $method)) {
			$output .= htmlBcp::$method($name, $params);
			$output .= htmlBcp::hidden('extra_field['.$this->name.']',array('value'=>$this->id));
		}
		return $output;
	}
	...
	public function processParams($tag, $id){
		...
		if ($this->name == "params") {
			...
			switch ($tag) {
				case 5: 
					$add_option = langBcp::_('Add Checkbox');
					$options_tag = '';
					$image_tag = ' style="display:none"';
				break;
				case 9: 
					$add_option = langBcp::_('Add Item');
					$options_tag = '';
					$image_tag = ' style="display:none"';
				break;
				case 12:
				...
				case 10:
					$options_tag = '';
					$add_option = langBcp::_('Add Radio Button');
					$image_tag = ' style="display:none"';
				break;
				case 8:
					$image_tag = '';
			...
			if ($tag > 0 || $id == 0) {
				$output .= '<div class="options options_tag"'.$options_tag.'>';
					...
					$output .= fieldAdapterBcp::_($id,'getExtraFieldOptions',fieldAdapterBcp::STR);
				$output .= '</div>';

				$output .= '<div class="options image_tag"'.$image_tag.'>'.langBcp::_('Dimensions').':<br />';
					$params->width?$width = $params->width:'';
					$params->height?$height = $params->height:'';
					$output .= langBcp::_('width').':<br />';
					$output .= htmlBcp::text('params[width]',array('value'=>$width)).'<br />';
					$output .= langBcp::_('height').':<br />';
					$output .= htmlBcp::text('params[height]',array('value'=>$height)).'<br />';
				$output .= '</div>';
			}
			if($this->adapt['htmlParams']) {
				$output .= fieldAdapterBcp::_($this, $this->adapt['htmlParams'], fieldAdapterBcp::STR);
			} else {
				$output .= '<a href="javascript:void(0);" class="set_properties">'.langBcp::_('Click to set field "id" and "class"').'</a>';
				$output .= '<div class="attributes" style="display:none;">'.langBcp::_('Attributes').':<br />';
				$output .= fieldAdapterBcp::_($params,'getFieldAttributes',  fieldAdapterBcp::STR);
				$output .= '</div>';
			}
			return $output;
		}
	...
	function checkVarFromParam($param, $element) {
		return utilsBcp::xmlAttrToStr($param, $element);
	  ...
	  // load xml structure of parameters
	   ...
	   foreach ($config->params->param as $param) {
		 // read the variables
		  ...
		  $helper = $this->checkVarFromParam($param,'helperBcp');
		  $module = $this->checkVarFromParam($param,'moduleBcp');
	...
	public function drawConfig($module, $additionalOptions = array()) {
		if(!frameBcp::_()->getModule($module)) 
			return false; 
		// check for xml file with params structure  
	   if(frameBcp::_()->getModule($module)->isExternal())
		   $config_xml = frameBcp::_()->getModule($module)->getModDir(). 'mod.xml';
	   else
		   $config_xml = BCP_MODULES_DIR.$module.DS.'mod.xml';

	   if (!file_exists($config_xml)) {
		   // if there is no configuration file for this $module
		   return langBcp::_('There are no configuration options for this module');
	   }
	   $output = '';
		   ...
		   foreach ($params as $param) {
			   ...
			   foreach ($configOptions as $key=>$value){
				  ...
				  // if helper is needed to render the object
					  ...
					  $helper = frameBcp::_()->getModule($hmodule)->getHelper();
					  if ($helper) {
				  ...
				  // filling the parameters to build html element
					 $htmlParams = array('value'=>$fieldValue,'optionsBcp'=>$options);
					 if($method == 'checkbox') {
						 $htmlParams['value'] = 1;
				  ...
				  // output label and html element
					 $output .= '<label>'.langBcp::_($configOptions[$key]['label']);
					 if ($configOptions[$key]['description'] != '') {
						 $output .= '<a class="toeOptTip" tip="'.langBcp::_($configOptions[$key]['description']).'"></a>';
					 }
					 $output .= '</label><br />';
					 $output .= htmlBcp::$method($name,$htmlParams).'<br />';
					 $output .= '</div>';
			   }
			   $i++;
			 $output .= '</div>';
