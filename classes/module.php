<?php
abstract class moduleBcp extends baseObjectBcp {
	...
	public function __construct($d, $params = array()) {
		...
		if(isset($d['ex_plug_dir']) && !empty($d['ex_plug_dir'])) {
			...
			$this->setExternalDir( utilsBcp::getExtModDir($d['ex_plug_dir']) );
			$this->setExternalPath( utilsBcp::getExtModPath($d['ex_plug_dir']) );
		}
	}
	...
	public function getModDir() {
		if(empty($this->_externalDir)) {
			return BCP_MODULES_DIR. $this->getCode(). DS;
		} else {
	...
	public function getModPath() {
		if(empty($this->_externalPath)) {
			return BCP_MODULES_PATH. $this->getCode(). '/';
		} else {
	...
	protected function _createController() {
		...
		if(file_exists($this->getModDir(). 'controller.php')) {
			...
			if(import($this->getModDir(). 'controller.php')) {
				$className = toeGetClassNameBcp($this->getCode(). 'Controller');
			}
			if(!empty($className)) {
	...
	protected function _createHelper() {
		...
		if (file_exists($this->getModDir().'helper.php')) {
			...
			importClassBcp($helper, $this->getModDir(). 'helper.php');
	...
	public function __call($name, $arguments) {
		...
		errorsBcp::push(langBcp::_(array('Module', $this->_code, 'method', $name, 'undefined')), errorsBcp::FATAL);
	}
