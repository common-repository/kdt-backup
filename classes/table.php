...
abstract class tableBcp {
    ...
    static public function getInstance($table = '') {
        ...
        if(!isset($instances[$table])) {
            $class = 'table'. strFirstUp($table). strFirstUp(BCP_CODE);
            ...
            if($id) {
                ...
                foreach($data as $field) {
                    ...
                    foreach($field as $k => $v) {
                        if(isset($this->_fields[$k])) {
                            $row[$k] = toeCreateObjBcp('fieldBcp', array(
                                    $this->_fields[$k]->name,
                                    $this->_fields[$k]->html,
                                    $this->_fields[$k]->type,
    ...
    public function getTable($transform = false) {
        if($transform)
            return dbBcp::prepareQuery($this->_table);
        else
            return $this->_table;
    }
    ...
    public function get($fields = '*', $where = '', $tables = '', $return = 'all') {
        ...
        return dbBcp::get($query, $return);
    }
    public function store($data, $method = 'INSERT', $where = '') {
		$this->_clearErrors();
        $method = strtoupper($method);
		if($this->_escape) {
			$data = dbBcp::escape($data);
		}
        $query = '';
        switch($method) {
            case 'INSERT':
        ...
        if(empty($fields)) {
            $this->_addError(langBcp::_('Nothig to update'));
            return false;
        }
        ...
        if(dbBcp::query($query)) {
            if($method == 'INSERT')
                return dbBcp::lastID();
            else
                return true;
        } else
			$this->_addError(langBcp::_('Database error. Please contact your developer.'));
        return false;
    }
            ...
            foreach($data as $k => $v) {
                if(array_key_exists($k, $this->_fields) || $k == $this->_id) {
                    ...
                    if($this->_fields[$k]->adapt['dbTo']) 
                        $val = fieldAdapterBcp::_($val, $this->_fields[$k]->adapt['dbTo'], fieldAdapterBcp::DB);
                    if($validate) {
                        if (is_object($this->_fields[$k])) {
                            $objForValidation = clone $this->_fields[$k];
                            $objForValidation->setValue($val);
                            if($errors = validatorBcp::_($objForValidation)) {
    ...
    /**
     * Add new fieldBcpBcp for children table (@see class field)
    ...
    protected function _addField($name, $html = 'text', $type = 'other', $default = '', $label = '', $maxlen = 0, $dbAdapt = '', $htmlAdapt = '', $description = '') {
        $this->_fields[$name] = toeCreateObjBcp('fieldBcp', array($name, $html, $type, $default, $label, $maxlen, $dbAdapt, $htmlAdapt, $description));
        return $this;
    }
    ...
    public function exists($value, $field = '') {
        if(!$field)
            $field = $this->_id;
        return dbBcp::get('SELECT '. $this->_id. ' FROM '. $this->_table. ' WHERE '. $field. ' = "'. $value. '"', 'one');
    }
            ...
            switch($f->type) {
                case 'date':
                    if($d[$key] == '0000-00-00' || empty($d[$key]))
                        ...
                        $d[$key] = date(BCP_DATE_FORMAT, dbBcp::dateToTime($d[$key]));
                    }
                    break;
                case 'int':
                case 'tinyint':
    ...
    public function getLastInsertID() {
        return dbBcp::get('SELECT MAX('. $this->_id. ') FROM '. $this->_table, 'one');
    }
