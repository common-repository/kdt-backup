...
class utilsBcp {
    ...
    static public function copyDirectories($source, $destination) {
        if(is_dir($source)) {
            ...
            while ( FALSE !== ( $readdirectory = $directory->read() ) ) {
                ...
                if (is_dir($PathDir)) {
                    utilsBcp::copyDirectories( $PathDir, $destination . '/' . $readdirectory );
                    continue;
                }
    ...
    static public function xmlNodeAttrsToArr($node) {
        ...
        foreach($node->attributes() as $a => $b) {
            $arr[$a] = utilsBcp::xmlAttrToStr($node, $a);
        }
        return $arr;
    }
    ...
    static public function getCurrentWPThemeDir() {
        static $themePath;
        if(empty($themePath)) {
            $themePath = get_theme_root(). DS. utilsBcp::getCurrentWPThemeCode(). DS; 
        }
        return $themePath;
    }
    /**
    ...
    static public function getExtModPath($plugName) {
        if(strpos($plugName, 'wp-content/themes') !== false) {  //for modules in theme dir
            return BCP_SITE_URL. str_replace(DS, '/', $plugName). '/';
        } else
    ...
    static public function isThisCommercialEdition() {
        ...
        foreach($commercialModules as $m) {
            if(!frameBcp::_()->getModule($m)) 
                return false;
            if(!is_dir(frameBcp::_()->getModule($m)->getModDir())) 
                return false;
        }
        return true;*/
        foreach(frameBcp::_()->getModules() as $m) {
	...
	static public function fileExists($filename) {
        ...
        public static function listToJson($array){
            return str_replace("'","\'", str_replace('\\','\\\\',utilsBcp::jsonEncode($array)));
        }
}
