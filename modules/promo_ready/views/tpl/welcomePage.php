<style type="text/css">
	...
	#bcpWelcomePageFindUsForm label {
		line-height: 24px;
		margin-left: 20px;
		font-size: 14px;
		display: block;
...
jQuery(document).ready(function(){
	jQuery('#bcpWelcomePageFindUsForm input[type=radio][name=where_find_us]').change(function(){
	...
	jQuery('#bcpWelcomePageFindUsForm').submit(function(){
...
<div class="container">
	<form id="bcpWelcomePageFindUsForm">
		<h1>
			<?php langBcp::_e('Welcome to')?>
			<?php echo BCP_WP_PLUGIN_NAME?>
			<?php langBcp::_e('Version')?>
			<?php echo BCP_VERSION?>!
		</h1>
            
		<div class="clear"></div>
		<div class="about-message">
			This is first start up of the <?php echo BCP_WP_PLUGIN_NAME?> plugin.<br />
		...
		<div class="plug-icon-shell">
			...
			<a target="_blank" href="http://readyshoppingcart.com/product/google-maps-plugin//"><?php echo BCP_WP_PLUGIN_NAME?></a><br />
			<a target="_blank" href="http://readyshoppingcart.com/product/google-maps-plugin/"><?php echo BCP_VERSION?></a><br />
		</div>
		<div class="clear"></div>
		...
		<?php foreach($this->askOptions as $askId => $askOpt) { ?>
			<label><?php echo htmlBcp::radiobutton('where_find_us', array('value' => $askId))?>&nbsp;<?php echo $askOpt['label']?></label>
			<?php if($askId == 4 /*Find on the web*/) { ?>
				<label id="toeFindUsUrlShell" style="display: none;">Please, post url: <?php echo htmlBcp::text('find_on_web_url')?></label>
			<?php } elseif($askId == 5 /*Other way*/) { ?>
				<label style="display: none;" id="toeOtherWayTextShell"><?php echo htmlBcp::textarea('other_way_desc')?></label>
			<?php }?>
		<?php }?>
		...
		<?php echo htmlBcp::hidden('mod', array('value' => 'promo_ready'))?>
		<?php echo htmlBcp::hidden('action', array('value' => 'welcomePageSaveInfo'))?>
		<?php echo htmlBcp::hidden('reqType', array('value' => 'ajax'))?>
		
                <br/>
                <br/>
                <?php echo htmlBcp::submit('gonext', array('value' => 'Thank for check info. Start using plugin.', 'attrs' => 'class="button button-primary button-hero"'))?>
		<?php echo htmlBcp::hidden('original_page', array('value' => reqBcp::getVar('page')))?>
